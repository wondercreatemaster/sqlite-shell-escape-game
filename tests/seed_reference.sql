...
-- Command strings are intentionally "messy" (extra whitespace, inline
-- `# comments`) and must be canonicalized as part of the migration.
...
-- Allowed commands: vetted coreutils utilities (wrapped in env, pipelines, env
-- assignments, absolute paths, and messy whitespace/comments) mixed with
-- wildcard/admin entries and unsafe/dangerous commands.
INSERT INTO allowed_commands (id, room_id, command) VALUES
 (1,  1, 'ls'),
 (2,  1, '  echo   hi  '),
 (3,  1, 'LC_ALL=C   sort   # sort'),
...
-- canonicalized; expected_command strings are also messy.
...
-- Doors: directed edges between rooms, each gated by a command that must be
-- runnable (survive hardening) in the source room for the door to be usable.
